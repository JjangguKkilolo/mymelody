...
이 앱의 캐릭터 그림은 사용자가 첨부한 도트 마이멜로디를 스타일 참고로 삼아, Codex의 **내장 imagegen** 도구로 새로 제작했습니다. 1.1.0에서는 추가로 제공한 의상·계란 도트를 참고해 기존 9종을 리메이크하고 양·계란 2종을 추가했습니다. 1.2.0의 공룡은 새 첨부 이미지에서 코스튬 콘셉트만 참고하고 기존 도트 그림체로 만들었습니다. 첨부 그림 자체나 서명을 앱 자산으로 복사하지 않았습니다. CLI/API 대체 경로는 사용하지 않았습니다.

## 최종 파일 규격

- 위치: `src/MyMelody.App/Assets/Characters/{id}.png`
- 12종 × 3단계 × 2표정 = 72개 프레임을 12개 PNG 시트에 담았습니다. 한 시트는 2열×3행, 전체 비율은 2:3입니다.
...
| dinosaur | 공룡 | 분홍 공룡 모자·작은 꼬리·크림 배 | 작은 날개·등 장식·노란 리본 | 노란 물방울무늬 알·민트 리본·앉은 자세 |
...
각 결과를 눈으로 확인한 뒤, PNG 픽셀 포맷·투명 배경 알파·2:3 시트 비율·개별 셀 규격·파일 수를 검사합니다. WPF 앱의 실제 렌더러로 기본 96 DIP에서 36개 모습을 모은 미리보기를 확인합니다. 코드로 캐릭터 그림을 대체하거나 배경을 지우지 않습니다.

생성 원본의 투명 픽셀에는 배경색 RGB가 남아 있을 수 있지만 알파 값은 0이며, WPF에서는 투명하게 표시됩니다. 원본 알파를 유지한 채 프로젝트에 복사했습니다. 이전 버전의 27개 시트는 Git 이력에 남고, 새 배포에는 현재 12개 시트만 포함합니다. 기존 캐릭터 ID는 그대로 유지하며 개인 기록을 변환하거나 초기화하지 않습니다.

1.1.1부터는 PNG 원본을 그대로 두고 앱 표시 단계에서 크기를 통일합니다. 알파 16/255 이상인 외형을 측정하고 테두리 2픽셀을 덧대어 도트 가장자리를 보존합니다. 모든 단계의 높이와 바닥선을 맞추되, 가장 넓은 의상도 들어오도록 공통 높이를 정합니다. 눈 뜬 모습과 감은 모습에는 같은 배율을 쓰고 가로 중심·바닥선을 각각 정렬해 원본 시트의 좌우 위치 차이를 보정합니다. WPF의 최근접 이웃 표시를 유지하며 새 그림 생성이나 파일 재인코딩은 하지 않습니다.

1.2.0에서는 그 공통 높이를 1.1.1 배포 당시 값으로 고정했습니다. 이후 추가되는 캐릭터도 같은 높이와 바닥선을 사용하며, 너무 넓은 소품으로 정해진 표시 폭을 넘으면 배포 검사가 거부합니다. 공룡을 추가하면서 기존 11개 PNG를 수정하거나 재생성하지 않았습니다.
